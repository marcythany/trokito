...
export function calculateOptimalChange(
	...
	// Validações básicas
	if (
		!isValidMonetaryValue(purchaseAmount) ||
		!isValidMonetaryValue(paidAmount)
	) {
	...
	const activeDenominations = getActiveDenominations(
		config.activeDenominations
	);

	// Aplica política de arredondamento
	const roundedChangeCents = applyRoundingPolicy(
		exactChangeCents,
		config.roundingPolicy
	);
	const roundingDifference = calculateRoundingDifference(
		exactChangeCents,
		roundedChangeCents
	);

	// Calcula a combinação de denominações
	const denominationCounts = calculateGreedyChange(
		roundedChangeCents,
		activeDenominations
	);

	// Verifica se conseguiu representar o troco completamente
	const totalRepresented = denominationCounts.reduce(
		(sum, item) => sum + item.denomination.value * item.count,
		0
	);

	const isOptimal =
		totalRepresented === roundedChangeCents && roundingDifference === 0;

	// Gera mensagem explicativa se houver arredondamento
	let message: string | undefined;
	if (roundingDifference !== 0) {
		if (roundingDifference > 0) {
			message = `Troco arredondado para cima em ${Math.abs(
				roundingDifference
			)} centavo(s)`;
		} else {
			message = `Cliente deve ${Math.abs(
				roundingDifference
			)} centavo(s) (dentro da tolerância)`;
		}
	}
...
export function separateNotesAndCoins(denominations: DenominationCount[]): {
	...
	const notes = denominations.filter(
		(item) => item.denomination.type === 'note'
	);
	const coins = denominations.filter(
		(item) => item.denomination.type === 'coin'
	);
