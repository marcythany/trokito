    ...
    <Card className={`border-2 border-accent ${className}`}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="text-center p-4 bg-card rounded-lg border border-border">
          ...
          <div className="text-4xl font-bold text-accent">{formatCurrency(summary.totalAmount)}</div>
          <div className="text-sm text-foreground/70 mt-1">{summary.totalPieces} peça(s)</div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center p-3 bg-card rounded-lg border border-border">
            ...
            <div className="text-2xl font-bold text-foreground">{formatCurrency(summary.totalNotes)}</div>
          </div>
          ...
          <div className="text-center p-3 bg-card rounded-lg border border-border">
            ...
            <div className="text-2xl font-bold text-foreground">{formatCurrency(summary.totalCoins)}</div>
          </div>
        </div>
        ...
        {summary.denominationCounts.length > 0 && (
          <>
            ...
            <div className="space-y-2">
              <h4 className="text-sm font-medium text-foreground/80">Detalhamento:</h4>
                ...
                {summary.denominationCounts.map((item, index) => (
                  <div key={index} className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-foreground">
                      {formatCurrency(item.denomination.value * item.count)}
                    </span>
                  </div>
                ))}
              </div>
